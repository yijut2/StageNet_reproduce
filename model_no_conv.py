...
class StageNet(nn.Module):
    ...
    def forward(self, input, time, device):
        ...
        for t in range(time_step):
            ...
            # local_h pre conv shape:  torch.Size([64, 384, 10])
            
            
            # local_h = self.nn_conv(local_h).squeeze(-1)
            local_h = self.replace_conv(local_h).squeeze(-1)
            # local_h after conv shape:  torch.Size([64, 384, 1])
            # local_h final shape:  torch.Size([64, 384])
